...
- users can expose a backend through LAN, their own Tailscale, MagicDNS, a manual HTTPS endpoint, or later Salchi Tunnel
- users can generate a hosted pairing link for `app.salchi.codes`
...
This plan intentionally leaves the paid Salchi cloud tunnel fabric out of scope. It defines the OSS foundation that Salchi Tunnel should later plug into.
...
- `https://app.salchi.codes` cannot reliably call `http://...` or `ws://...` endpoints because browsers block mixed content.
...
- Generate `app.salchi.codes` links only from endpoints that are likely hosted-browser compatible, or show a warning with an explicit fallback.
...
```text
https://app.salchi.codes/pair?host=<encoded endpoint httpBaseUrl>#token=<one-time token>
```
...
- HTTP private-network endpoints are marked incompatible with `app.salchi.codes`.
...
- `app.salchi.codes` works as a real client shell.
...
- `app.salchi.codes` can pair a reachable HTTPS backend and reconnect after reload.
- A saved environment can be used without any backend at `app.salchi.codes`.
...
## Phase 4: Future Salchi Tunnel Provider
...
- create endpoint with `provider: "salchi-tunnel"`
- surface tunnel status
- provide stable HTTPS URL
- use existing backend pairing/session auth
- never bypass server auth

The tunnel fabric can later be Pipenet-derived, Tailscale-derived, or another reverse tunnel implementation. The rest of Salchi should only see an `AdvertisedEndpoint`.

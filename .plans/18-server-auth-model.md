...
Define the long-term server auth architecture for Salchi before first-class remote environments ship.
...
Every privileged surface of the Salchi server must go through the same auth policy engine:
...
Participants:
  ...
  SalchiServer      = local backend child process
...
DesktopMain -> SalchiServer : spawn server (--bootstrap-fd ...)
DesktopMain -> SalchiServer : send desktop bootstrap envelope
note over SalchiServer : policy = DesktopManagedLocalPolicy
note over SalchiServer : allowed pairing = desktop-bootstrap only

Frontend -> DesktopMain : request local bootstrap grant
DesktopMain --> Frontend : short-lived desktop bootstrap grant

Frontend -> SalchiServer : POST /api/auth/bootstrap
SalchiServer -> SalchiServer : validate desktop bootstrap grant
SalchiServer -> SalchiServer : create browser session
SalchiServer --> Frontend : Set-Cookie: session=...

Frontend -> SalchiServer : GET /ws + authenticated cookie
SalchiServer -> SalchiServer : validate cookie session
SalchiServer --> Frontend : websocket accepted
```
...
```text
Participants:
  UserShell     = npx salchi launcher
  SalchiServer      = standalone local server
  Browser       = browser tab

UserShell -> SalchiServer : start server
SalchiServer -> SalchiServer : getOrCreate("server-signing-key")
note over SalchiServer : policy = LoopbackBrowserPolicy

UserShell -> SalchiServer : issue one-time pairing token
SalchiServer --> UserShell : pairing URL or pairing token

UserShell --> Browser : open /pair?token=...

Browser -> SalchiServer : GET /pair?token=...
SalchiServer -> SalchiServer : validate one-time token
SalchiServer -> SalchiServer : create browser session
SalchiServer --> Browser : Set-Cookie: session=...
SalchiServer --> Browser : redirect to app

Browser -> SalchiServer : GET /ws + authenticated cookie
SalchiServer --> Browser : websocket accepted
```
...
Participants:
  ...
  SalchiServer      = Salchi server
  PhoneBrowser  = mobile browser

DesktopUser -> DesktopMain : enable remote access via tunnel
DesktopMain -> SalchiServer : switch policy to RemoteReachablePolicy
DesktopMain -> Tunnel : publish local Salchi endpoint
Tunnel --> DesktopMain : public https/wss URL

DesktopMain -> SalchiServer : issue one-time pairing token
SalchiServer --> DesktopMain : pairing token
...
Tunnel -> SalchiServer : forward request
SalchiServer -> SalchiServer : validate one-time token
SalchiServer -> SalchiServer : create mobile browser session
SalchiServer --> PhoneBrowser : Set-Cookie: session=...
SalchiServer --> PhoneBrowser : redirect to app

PhoneBrowser -> Tunnel : GET /ws + authenticated cookie
Tunnel -> SalchiServer : forward websocket upgrade
SalchiServer --> PhoneBrowser : websocket accepted
```
...
DesktopUser -> SalchiServer : enable private-network access
SalchiServer -> SalchiServer : switch policy to RemoteReachablePolicy
DesktopUser -> SalchiServer : issue one-time pairing token
SalchiServer --> DesktopUser : pairing URL / QR

DesktopUser -> PhoneBrowser : open private-network URL
PhoneBrowser -> PrivateNet : GET http(s)://private-host/pair?token=...
PrivateNet -> SalchiServer : route request
SalchiServer -> SalchiServer : validate one-time token
SalchiServer -> SalchiServer : create mobile browser session
SalchiServer --> PhoneBrowser : Set-Cookie: session=...
SalchiServer --> PhoneBrowser : redirect to app
...
PrivateNet -> SalchiServer : websocket upgrade
SalchiServer --> PhoneBrowser : websocket accepted
```
...
Participants:
  ...
  RemoteSalchi      = remote Salchi server
  Frontend      = desktop renderer

DesktopUser -> DesktopMain : add SSH host
DesktopMain -> SSH : connect to remote host
SSH -> RemoteHost : probe environment / verify salchi availability
DesktopMain -> SSH : run remote launch command
SSH -> RemoteHost : salchi remote launch --json
RemoteHost -> RemoteSalchi : start or reuse server
RemoteSalchi --> RemoteHost : port + environment metadata
...
note over RemoteSalchi : policy = RemoteReachablePolicy
note over DesktopMain,RemoteSalchi : desktop may use a trusted bootstrap flow here

Frontend -> DesktopMain : request bootstrap for selected environment
DesktopMain --> Frontend : short-lived bootstrap grant

Frontend -> RemoteSalchi : POST /api/auth/bootstrap via forwarded port
RemoteSalchi -> RemoteSalchi : validate bootstrap grant
RemoteSalchi -> RemoteSalchi : create browser session
RemoteSalchi --> Frontend : Set-Cookie: session=...

Frontend -> RemoteSalchi : GET /ws + authenticated cookie
RemoteSalchi --> Frontend : websocket accepted
```

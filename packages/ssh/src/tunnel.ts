...
import { extractJsonObject, fromLenientJson } from "@salchi/shared/schemaJson";
import { satisfiesSemverRange } from "@salchi/shared/semver";
...
export interface RemoteSalchiRunnerOptions {
...
export interface SshEnvironmentManagerOptions {
  ...
  readonly resolveCliRunner?: Effect.Effect<RemoteSalchiRunnerOptions>;
}
...
function sshRunnerLogFields(runner: RemoteSalchiRunnerOptions | undefined) {
...
remote_node_satisfies_engine() {
  SALCHI_NODE_ENGINE_RANGE=@@SALCHI_NODE_ENGINE_RANGE@@
  if [ -z "$SALCHI_NODE_ENGINE_RANGE" ]; then
    return 0
  fi
  node - "$SALCHI_NODE_ENGINE_RANGE" <<'NODE'
@@SALCHI_NODE_ENGINE_CHECK_SCRIPT@@
NODE
}
...
ensure_remote_node_path() {
  ...
  if ! command -v node >/dev/null 2>&1 && [ -d "$NVM_DIR/versions/node" ]; then
    for SALCHI_NODE_BIN in "$NVM_DIR"/versions/node/*/bin; do
      if [ -x "$SALCHI_NODE_BIN/node" ]; then
        PATH="$SALCHI_NODE_BIN:$PATH"
        export PATH
      fi
    done
  fi
...
set -eu
@@SALCHI_NODE_ENV_SCRIPT@@
ensure_remote_node_path || true
SALCHI_NODE_SCRIPT_PATH=@@SALCHI_NODE_SCRIPT_PATH@@
if [ -n "$SALCHI_NODE_SCRIPT_PATH" ]; then
  ...
  exec node "$SALCHI_NODE_SCRIPT_PATH" "$@"
fi
if command -v salchi >/dev/null 2>&1; then
  exec salchi "$@"
fi
if command -v npx >/dev/null 2>&1; then
  exec npx --yes @@SALCHI_PACKAGE_SPEC@@ "$@"
fi
if command -v npm >/dev/null 2>&1; then
  exec npm exec --yes @@SALCHI_PACKAGE_SPEC@@ -- "$@"
fi
printf 'Remote host is missing the salchi CLI and could not install @@SALCHI_PACKAGE_SPEC@@ because node/npm/npx are unavailable on PATH. Install Node or configure a supported version manager for non-interactive shells.\\n' >&2
exit 1
`;
...
@@SALCHI_NODE_ENV_SCRIPT@@
STATE_KEY="$1"
STATE_DIR="$HOME/.salchi/ssh-launch/$STATE_KEY"
DEFAULT_SERVER_HOME="$HOME/.salchi"
DEFAULT_RUNTIME_FILE="$DEFAULT_SERVER_HOME/userdata/server-runtime.json"
PORT_FILE="$STATE_DIR/port"
PID_FILE="$STATE_DIR/pid"
MANAGED_FILE="$STATE_DIR/managed"
LOG_FILE="$STATE_DIR/server.log"
RUNNER_FILE="$STATE_DIR/run-salchi.sh"
RUNNER_NEXT="$STATE_DIR/run-salchi.next.$$"
mkdir -p "$STATE_DIR"
cleanup_runner_next() {
  rm -f "$RUNNER_NEXT"
}
...
pick_port() {
  node - "$PORT_FILE" "@@SALCHI_DEFAULT_REMOTE_PORT@@" "@@SALCHI_REMOTE_PORT_SCAN_WINDOW@@" <<'NODE'
@@SALCHI_PICK_PORT_SCRIPT@@
NODE
}
wait_ready() {
  node - "$REMOTE_PORT" "$1" "@@SALCHI_READY_PROBE_TIMEOUT_MS@@" <<'NODE'
@@SALCHI_WAIT_READY_SCRIPT@@
NODE
}
wait_for_pid_exit() {
  PID_TO_WAIT="$1"
...
if [ -n "$DEFAULT_REMOTE_PORT" ]; then
  ...
  if wait_ready "@@SALCHI_REUSE_READY_TIMEOUT_MS@@"; then
...
if [ "$REMOTE_MANAGED" = "external" ]; then
  if [ -z "$REMOTE_PORT" ] || ! wait_ready "@@SALCHI_REUSE_READY_TIMEOUT_MS@@"; then
    REMOTE_PID=""
    REMOTE_PORT=""
    REMOTE_MANAGED=""
  fi
  ...
  if [ "$RUNNER_CHANGED" -eq 1 ]; then
    ...
    REMOTE_PORT=""
    REMOTE_MANAGED=""
  elif ! wait_ready "@@SALCHI_REUSE_READY_TIMEOUT_MS@@"; then
    kill "$REMOTE_PID" 2>/dev/null || true
    wait_for_pid_exit "$REMOTE_PID"
    REMOTE_PID=""
    REMOTE_PORT=""
...
if [ -z "$REMOTE_PORT" ]; then
  ...
  nohup env SALCHI_NO_BROWSER=1 "$RUNNER_FILE" serve --host 127.0.0.1 --port "$REMOTE_PORT" --base-dir "$DEFAULT_SERVER_HOME" >>"$LOG_FILE" 2>&1 < /dev/null &
  REMOTE_PID="$!"
  printf '%s\\n' "$REMOTE_PID" >"$PID_FILE"
  printf '%s\\n' "$REMOTE_PORT" >"$PORT_FILE"
  printf 'managed\\n' >"$MANAGED_FILE"
  if ! wait_ready "@@SALCHI_READY_TIMEOUT_MS@@"; then
    printf 'Remote Salchi server did not become ready on 127.0.0.1:%s.\\n' "$REMOTE_PORT" >&2
...
STATE_DIR="$HOME/.salchi/ssh-launch/@@SALCHI_STATE_KEY@@"
DEFAULT_SERVER_HOME="$HOME/.salchi"
RUNNER_FILE="$STATE_DIR/run-salchi.sh"
mkdir -p "$STATE_DIR"
cat >"$RUNNER_FILE" <<'SH'
...
export function buildRemoteSalchiRunnerScript(input?: RemoteSalchiRunnerOptions): string {
  const packageSpec = shellSingleQuote(input?.packageSpec?.trim() || "salchi@latest");
  const nodeScriptPath = input?.nodeScriptPath?.trim() || "";
  return stripTrailingNewlines(
    applyScriptPlaceholders(REMOTE_RUNNER_SCRIPT, {
      SALCHI_PACKAGE_SPEC: packageSpec,
      SALCHI_NODE_SCRIPT_PATH: shellSingleQuote(nodeScriptPath),
      SALCHI_NODE_ENV_SCRIPT: buildRemoteNodeEnvScript(input),
    }),
  );
}

function buildRemoteNodeEnvScript(input?: RemoteSalchiRunnerOptions): string {
  return stripTrailingNewlines(
    applyScriptPlaceholders(REMOTE_NODE_ENV_SCRIPT, {
      SALCHI_NODE_ENGINE_RANGE: shellSingleQuote(input?.nodeEngineRange?.trim() || ""),
      SALCHI_NODE_ENGINE_CHECK_SCRIPT: stripTrailingNewlines(buildRemoteNodeEngineCheckScript()),
    }),
  );
}

export function buildRemoteLaunchScript(input?: RemoteSalchiRunnerOptions): string {
  return applyScriptPlaceholders(REMOTE_LAUNCH_SCRIPT, {
    SALCHI_NODE_ENV_SCRIPT: buildRemoteNodeEnvScript(input),
    SALCHI_RUNNER_SCRIPT: stripTrailingNewlines(buildRemoteSalchiRunnerScript(input)),
    SALCHI_PICK_PORT_SCRIPT: stripTrailingNewlines(REMOTE_PICK_PORT_SCRIPT),
    SALCHI_WAIT_READY_SCRIPT: stripTrailingNewlines(REMOTE_WAIT_READY_SCRIPT),
    SALCHI_DEFAULT_REMOTE_PORT: String(DEFAULT_REMOTE_PORT),
    SALCHI_REMOTE_PORT_SCAN_WINDOW: String(REMOTE_PORT_SCAN_WINDOW),
    SALCHI_READY_TIMEOUT_MS: String(REMOTE_READY_TIMEOUT_MS),
    SALCHI_REUSE_READY_TIMEOUT_MS: String(REMOTE_REUSE_READY_TIMEOUT_MS),
    SALCHI_READY_PROBE_TIMEOUT_MS: String(SSH_READY_PROBE_TIMEOUT_MS),
  });
}

export function buildRemotePairingScript(
  target: DesktopSshEnvironmentTarget,
  input?: RemoteSalchiRunnerOptions,
): string {
  return applyScriptPlaceholders(REMOTE_PAIRING_SCRIPT, {
    SALCHI_STATE_KEY: remoteStateKey(target),
    SALCHI_RUNNER_SCRIPT: stripTrailingNewlines(buildRemoteSalchiRunnerScript(input)),
  });
}
...
export const launchOrReuseRemoteServer = Effect.fn("ssh/tunnel.launchOrReuseRemoteServer")(
  function* (
    ...
    runner?: RemoteSalchiRunnerOptions,
  ): Effect.fn.Return<
...
const makeSshEnvironmentManager = Effect.fn("ssh/tunnel.SshEnvironmentManager.make")(function* (
  ...
  const createTunnelEntry = Effect.fn("ssh/tunnel.ensureTunnelEntry.create")(function* (input: {
    readonly key: string;
    readonly resolvedTarget: DesktopSshEnvironmentTarget;
    readonly runner?: RemoteSalchiRunnerOptions;
...
>()("@salchi/ssh/tunnel/SshEnvironmentManager") {

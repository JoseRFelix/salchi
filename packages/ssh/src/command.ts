...
import type { DesktopSshEnvironmentTarget, DesktopUpdateChannel } from "@salchi/contracts";
...
const PUBLISHABLE_SALCHI_VERSION_PATTERN = /^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/u;
...
export function resolveRemoteSalchiCliPackageSpec(input: {
  readonly appVersion: string;
  readonly updateChannel: DesktopUpdateChannel;
  readonly isDevelopment?: boolean;
}): string {
  const appVersion = input.appVersion.trim();
  if (!input.isDevelopment && PUBLISHABLE_SALCHI_VERSION_PATTERN.test(appVersion)) {
    return `salchi@${appVersion}`;
  }

  if (input.isDevelopment) {
    return "salchi@nightly";
  }

  return input.updateChannel === "nightly" ? "salchi@nightly" : "salchi@latest";
}

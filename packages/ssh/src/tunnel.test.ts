...
describe("ssh tunnel scripts", () => {
  it("builds the remote salchi runner with npx and npm fallbacks", () => {
    const script = buildRemoteSalchiRunnerScript({ nodeEngineRange: TEST_NODE_ENGINE_RANGE });

    assert.include(script, "SALCHI_NODE_SCRIPT_PATH=''");
    assert.include(script, 'exec salchi "$@"');
    assert.include(script, "exec npx --yes 'salchi@latest' \"$@\"");
    assert.include(script, "exec npm exec --yes 'salchi@latest' -- \"$@\"");
    assert.include(script, "could not install 'salchi@latest'");
    assert.include(script, 'prepend_path_if_dir "$HOME/.local/bin"');
    assert.include(script, `SALCHI_NODE_ENGINE_RANGE='${TEST_NODE_ENGINE_RANGE}'`);
    ...
    assert.include(script, 'for SALCHI_NODE_BIN in "$NVM_DIR"/versions/node/*/bin');
    assert.notInclude(script, "ensure $NVM_DIR/nvm.sh is available");
  });

  it("does not hard-code a remote node engine range", () => {
    const script = buildRemoteSalchiRunnerScript();

    assert.include(script, "SALCHI_NODE_ENGINE_RANGE=''");
    assert.notInclude(script, TEST_NODE_ENGINE_RANGE);
  });

  it("shell-quotes package specs in the remote salchi runner", () => {
    const script = buildRemoteSalchiRunnerScript({
      packageSpec: "salchi@nightly; touch /tmp/salchi-owned",
    });

    assert.include(script, "exec npx --yes 'salchi@nightly; touch /tmp/salchi-owned' \"$@\"");
    assert.include(
      script,
      "exec npm exec --yes 'salchi@nightly; touch /tmp/salchi-owned' -- \"$@\"",
    );
    assert.notInclude(script, "exec npx --yes salchi@nightly; touch /tmp/salchi-owned");
  });

  it("builds the remote salchi runner with a node script override", () => {
    const script = buildRemoteSalchiRunnerScript({
    ...
    assert.include(
      script,
      "SALCHI_NODE_SCRIPT_PATH='/Users/julius/Development/Work/codething-mvp/apps/server/dist/bin.mjs'",
    );
    assert.include(script, 'exec node "$SALCHI_NODE_SCRIPT_PATH" "$@"');
  });

  it("uses the remote salchi runner for launch and pairing scripts", () => {
    const target = {
      alias: "devbox",
    ...
    assert.include(
      buildRemoteLaunchScript({ nodeEngineRange: TEST_NODE_ENGINE_RANGE }),
      `SALCHI_NODE_ENGINE_RANGE='${TEST_NODE_ENGINE_RANGE}'`,
    );
    assert.include(
      buildRemoteLaunchScript({ nodeEngineRange: TEST_NODE_ENGINE_RANGE }),
    ...
    assert.include(buildRemoteLaunchScript(), "Remote Salchi server did not become ready");
    assert.include(buildRemoteLaunchScript({ packageSpec: "salchi@nightly" }), "salchi@nightly");
    assert.include(
    ...
    assert.include(
      buildRemotePairingScript(target, { packageSpec: "salchi@nightly" }),
      "salchi@nightly",
    );
    assert.include(

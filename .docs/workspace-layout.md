...
- `/apps/desktop`: Electron shell. Spawns a desktop-scoped `salchi` backend process and loads the shared web app.
...
- `/packages/shared`: Shared runtime utilities consumed by both server and web. Uses explicit subpath exports (e.g. `@salchi/shared/git`, `@salchi/shared/DrainableWorker`) — no barrel index.

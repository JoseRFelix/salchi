...
This document describes the target architecture for first-class remote environments in Salchi.
...
- Keep the Salchi server as the execution boundary.
...
Salchi already has a clean runtime boundary: the client talks to a Salchi server over HTTP/WebSocket, and the server owns orchestration, providers, terminals, git, and filesystem operations.
...
└───────────────┬──────────────────────────────┘
                │
                │ connects to one Salchi server
                │
┌───────────────▼──────────────────────────────┐
│ Execution environment = one Salchi server       │
│                                              │
...
The important decision is that remoteness is expressed at the environment connection layer, not by splitting the Salchi runtime itself.

## Domain model

### ExecutionEnvironment

An `ExecutionEnvironment` is one running Salchi server instance.
...
How does the client speak WebSocket to a Salchi server?
...
- best fit for hosted or self-managed remote Salchi deployments
...
For Salchi, tunnels are best modeled as another `AccessEndpoint`, not as a different kind of environment.
...
Tailscale-backed access sits here architecturally even though the current implementation is endpoint discovery rather than a Salchi-managed tunnel. It contributes private-network endpoints and lets the existing HTTP/WebSocket client path do the actual connection.
...
- reach a machine
- probe it
- launch or reuse a remote Salchi server
...
How does a Salchi server come to exist on the target machine?
...
The user or operator already runs Salchi on the target machine, and the client connects through a direct or tunneled WebSocket endpoint.
...
What should be different in Salchi:
...
The recommended Salchi flow is:

1. Desktop connects over SSH.
2. Desktop probes the remote machine and verifies Salchi availability.
3. Desktop launches or reuses a remote Salchi server.
...
This is the inverse of remote launch: a local Salchi server is already running, and the client publishes it through a tunnel.
...
- A manually hosted Salchi server might be reached through direct `wss`.
...
Salchi already supports a WebSocket auth token on the server. That should become a first-class part of environment access rather than remaining an incidental query parameter convention.
...
Salchi should not copy that part.

Salchi already has the right runtime boundary:

- one Salchi server per environment
- ordinary HTTP/WebSocket between client and environment

So Salchi should borrow Zed's launch discipline, not its transport protocol.

...
Salchi has one server-side observability model:
...
Completed spans are written as NDJSON records to `serverTracePath` (by default, `~/.salchi/userdata/logs/server.trace.ndjson`).
...
```bash
export SALCHI_OTLP_TRACES_URL=http://localhost:4318/v1/traces
export SALCHI_OTLP_METRICS_URL=http://localhost:4318/v1/metrics
export SALCHI_OTLP_SERVICE_NAME=salchi-local
```

Optional:

```bash
export SALCHI_TRACE_MIN_LEVEL=Info
export SALCHI_TRACE_TIMING_ENABLED=true
```
...
Launch the actual app executable from the same shell so the desktop app and embedded backend inherit `SALCHI_OTLP_*`.

macOS app bundle example:

```bash
SALCHI_OTLP_TRACES_URL=http://localhost:4318/v1/traces \
SALCHI_OTLP_METRICS_URL=http://localhost:4318/v1/metrics \
SALCHI_OTLP_SERVICE_NAME=salchi-desktop \
"/Applications/Salchi.app/Contents/MacOS/Salchi"
```
...
```bash
tail -f "$SALCHI_HOME/userdata/logs/server.trace.ndjson"
```
...
jq -c 'select(.exit._tag != "Success") | {
  ...
  exit,
  attributes
}' "$SALCHI_HOME/userdata/logs/server.trace.ndjson"
```
...
jq -r 'select(.traceId == "TRACE_ID_HERE") | [
  ...
  (.parentSpanId // "-"),
  .durationMs
] | @tsv' "$SALCHI_HOME/userdata/logs/server.trace.ndjson"
```
...
- service name such as `salchi-local`, `salchi-dev`, or `salchi-desktop`
...
- `salchi_rpc_request_duration`
- `salchi_orchestration_command_duration`
- `salchi_orchestration_command_ack_duration`
- `salchi_provider_turn_duration`
- `salchi_git_command_duration`
- `salchi_db_query_duration`

Counters tell you volume and failure rate:

- `salchi_rpc_requests_total`
- `salchi_orchestration_commands_total`
- `salchi_provider_turns_total`
- `salchi_git_commands_total`
- `salchi_db_queries_total`
...
`salchi_orchestration_command_ack_duration` measures:
...
1. Check `salchi_orchestration_command_ack_duration` by `commandType`.
...
- `SALCHI_OTLP_TRACES_URL` was not set
...
- `SALCHI_TRACE_FILE`: override trace file path
- `SALCHI_TRACE_MAX_BYTES`: per-file rotation size, default `10485760`
- `SALCHI_TRACE_MAX_FILES`: rotated file count, default `10`
...
- `SALCHI_OTLP_TRACES_URL`: OTLP trace endpoint
- `SALCHI_OTLP_METRICS_URL`: OTLP metric endpoint
- `SALCHI_OTLP_EXPORT_INTERVAL_MS`: export interval, default `10000`
- `SALCHI_OTLP_SERVICE_NAME`: service name, default `salchi-server`

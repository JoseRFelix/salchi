...
- Publishes the CLI package (`apps/server`, npm package `salchi`) with OIDC trusted publishing from the same workflow file:
...
- `SALCHI_WEB_ROUTER_URL`: defaults to `https://app.salchi.codes`.
- `SALCHI_WEB_LATEST_DOMAIN`: defaults to `latest.app.salchi.codes`.
- `SALCHI_WEB_NIGHTLY_DOMAIN`: defaults to `nightly.app.salchi.codes`.

Required Vercel domains:

- `app.salchi.codes`: the router domain users open, updated by stable releases.
- `latest.app.salchi.codes`: channel alias updated by stable releases.
- `nightly.app.salchi.codes`: channel alias updated by nightly releases.

The router domain uses `apps/web/vercel.ts` routes. Users opt into a channel by
visiting `/__salchi/channel?channel=latest` or
`/__salchi/channel?channel=nightly`; the router stores the
`salchi_web_channel` cookie and rewrites future requests on `app.salchi.codes` to
...
through `/__salchi/channel` on the router domain so the user's channel cookie is
...
4. Run one stable release deployment, or manually alias the current stable
   deployment, so `app.salchi.codes` points at a deployment containing the router
...
- Publishes the CLI package (`apps/server`, npm package `salchi`) to the `nightly` npm dist-tag using the same nightly version.
...
- Repository slug source:
  - `SALCHI_DESKTOP_UPDATE_REPOSITORY` (format `owner/repo`), if set.
  - otherwise `GITHUB_REPOSITORY` from GitHub Actions.
- Temporary private-repo auth workaround:
  - set `SALCHI_DESKTOP_UPDATE_GITHUB_TOKEN` (or `GH_TOKEN`) in the desktop app runtime environment.
...
1. Confirm npm org/user owns package `salchi` (or rename package first if needed).

...
This guide is for people who want to use more than one Codex account in Salchi.
...
- both accounts can see the same Salchi/Codex sessions
...
In Salchi Settings, name it something obvious:
...
In Salchi Settings, add another Codex provider:
...
Salchi shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, Salchi treats it

...
it.layer(NodeServices.layer)("bin cli parsing", (it) => {
  ...
  it.effect("executes auth pairing subcommands and redacts secrets from list output", () =>
    Effect.gen(function* () {
      const baseDir = mkdtempSync(join(tmpdir(), "salchi-cli-auth-pairing-test-"));
  ...
  it.effect("executes auth session subcommands and redacts secrets from list output", () =>
    Effect.gen(function* () {
      const baseDir = mkdtempSync(join(tmpdir(), "salchi-cli-auth-session-test-"));
  ...
  it.effect("adds, renames, and removes projects offline through the orchestration engine", () =>
    Effect.gen(function* () {
      const baseDir = mkdtempSync(join(tmpdir(), "salchi-cli-projects-offline-test-"));
      const workspaceRoot = mkdtempSync(join(tmpdir(), "salchi-cli-projects-workspace-"));
  ...
  it.effect("routes project commands through a running server when runtime state is present", () =>
    Effect.gen(function* () {
      const baseDir = mkdtempSync(join(tmpdir(), "salchi-cli-projects-live-test-"));
      const workspaceRoot = mkdtempSync(join(tmpdir(), "salchi-cli-projects-live-workspace-"));
  ...
  it.effect("rejects dev-url on project commands", () =>
    Effect.gen(function* () {
      const workspaceRoot = mkdtempSync(
        join(tmpdir(), "salchi-cli-projects-unknown-option-workspace-"),
      );

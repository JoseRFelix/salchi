...
it("marks the most preferred available built-in Codex model as default", () => {
  ...
  assert.deepStrictEqual(
    ...
    [
      { slug: "gpt-5.6-terra", isDefault: undefined },
...
it("keeps Codex's declared default when no preferred model is available", () => {
  const models = applyPreferredCodexDefaultModel([
  ...
  assert.equal(models.find((model) => model.isDefault)?.slug, "gpt-5.4");
});

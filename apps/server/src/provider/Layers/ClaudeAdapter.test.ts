...
describe("ClaudeAdapterLive", () => {
  ...
  it.effect("uses high as the Claude Opus 5 default effort", () => {
    const harness = makeHarness();
    return Effect.gen(function* () {
      const adapter = yield* ClaudeAdapter;
      yield* adapter.startSession({
  ...
  it.effect("does not complete a turn for a result received without active turn state", () => {
    const harness = makeHarness();
    return Effect.gen(function* () {
      const adapter = yield* ClaudeAdapter;
      ...
      harness.query.emit({
        ...
        errors: [],
        num_turns: 1,
        session_id: "sdk-session-result-guard",
        uuid: "result-real-turn",
      } as unknown as SDKMessage);
      harness.query.emit({
        type: "result",
        subtype: "success",
  ...
  it.effect("stops retrying and fails the turn on a Claude authentication retry", () => {
    const harness = makeHarness();
    return Effect.gen(function* () {
      const adapter = yield* ClaudeAdapter;
        ...
        const runtimeEventsFiber = yield* adapter.streamEvents.pipe(
          Stream.filter(
            (event) =>
              ...
              event.payload.usage.totalProcessedTokens === 535000,
          ),
          Stream.take(1),
          Stream.runCollect,
          Effect.forkChild,

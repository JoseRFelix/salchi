...
import { deepMerge } from "@salchi/shared/Struct";
import { createModelCapabilities } from "@salchi/shared/model";
import { applyServerSettingsPatch } from "@salchi/shared/serverSettings";
...
const TestProviderInstanceRegistryHydrationLive = ProviderInstanceRegistryHydrationLive.pipe(
  Layer.provide(BrowserAgentBrokerDisabled),
);
...
process.env.SALCHI_CURSOR_ENABLED = "1";
...
const waitForProviderSnapshot = (
  ...
  Effect.scoped(
    Effect.gen(function* () {
      ...
      const findMatching = (providers: ReadonlyArray<ServerProvider>) => {
        const provider = providers.find((candidate) => candidate.instanceId === instanceId);
        return provider !== undefined && predicate(provider) ? provider : undefined;
      };

      const initial = findMatching(yield* registry.getProviders);
      if (initial !== undefined) {
        return initial;
      }

      while (true) {
        const providerUpdates = yield* pull;
        for (const providers of providerUpdates) {
          const match = findMatching(providers);
          if (match !== undefined) {
            return match;
          }
        }
...
it.layer(Layer.mergeAll(NodeServices.layer, ServerSettingsService.layerTest(), TestHttpClientLive))(
  ...
  (it) => {
    ...
    describe("ProviderRegistryLive", () => {
            ...
            ProviderRegistryLive.pipe(
              ...
              Layer.provideMerge(
                ServerConfig.layerTest(process.cwd(), {
                  prefix: "salchi-provider-registry-background-refresh-",
                }),
              ),
        ...
        Effect.gen(function* () {
          ...
          const runtimeServices = yield* Layer.build(
            ProviderRegistryLive.pipe(
              ...
              Layer.provideMerge(
                ServerConfig.layerTest(process.cwd(), {
                  prefix: "salchi-provider-registry-merged-persist-",
                }),
              ),
              Layer.provideMerge(NodeServices.layer),
            ),
        ...
        Effect.gen(function* () {
          ...
          const runtimeServices = yield* Layer.build(
            ProviderRegistryLive.pipe(
              ...
              Layer.provideMerge(
                ServerConfig.layerTest(process.cwd(), {
                  prefix: "salchi-provider-registry-refresh-failure-",
                }),
              ),
              Layer.provideMerge(NodeServices.layer),
            ),
      ...
      it.effect("does not log interrupt-only refreshes during registry sync", () => {
        ...
        return Effect.gen(function* () {
          ...
          const runtimeServices = yield* Layer.build(
            ProviderRegistryLive.pipe(
              ...
              Layer.provideMerge(
                ServerConfig.layerTest(process.cwd(), {
                  prefix: "salchi-provider-registry-refresh-interrupt-",
                }),
              ),
              Layer.provideMerge(NodeServices.layer),
            ),
        ...
        Effect.gen(function* () {
          ...
          const runtimeServices = yield* Layer.build(
            ProviderRegistryLive.pipe(
              ...
              Layer.provideMerge(
                ServerConfig.layerTest(process.cwd(), {
                  prefix: "salchi-provider-registry-sync-failure-",
                }),
              ),
              Layer.provideMerge(NodeServices.layer),
            ),
      ...
      it.effect("propagates real Codex probe failures to the aggregator at boot", () =>
        Effect.gen(function* () {
          const missingBinary = `salchi_codex_missing_`;
          const serverSettings = yield* makeMutableServerSettingsService(
            decodeServerSettings(
              deepMerge(encodedDefaultServerSettings, {
                ...
                providerInstances: {
                  // Matches the shape the user had in `.salchi/dev/settings.json`
          ...
          const providerRegistryLayer = ProviderRegistryLive.pipe(
            ...
            Layer.provideMerge(
              ServerConfig.layerTest(process.cwd(), {
                prefix: "salchi-provider-registry-",
              }),
            ),
          ...
          yield* Effect.gen(function* () {
            const registry = yield* ProviderRegistry;
            const codexPersonal = yield* waitForProviderSnapshot(
              registry,
      ...
      it.effect("re-probes when settings change the codex binaryPath", () =>
        Effect.gen(function* () {
          const firstMissing = `salchi_codex_first_`;
          const secondMissing = `salchi_codex_second_`;
          const serverSettings = yield* makeMutableServerSettingsService(
            decodeServerSettings(
              deepMerge(encodedDefaultServerSettings, {
                providers: {
          ...
          yield* Effect.gen(function* () {
            ...
            const initialCodex = yield* waitForProviderSnapshot(
              registry,
            ...
            // Let the settings watcher fiber subscribe before publishing the
            // mutation, then advance the virtual clock so `checkedAt` can
            // distinguish the fresh probe from the boot probe.
            yield* Effect.yieldNow;
            ...
            const reprobedCodex = yield* waitForProviderSnapshot(
              registry,
              ProviderInstanceId.make("codex"),
              (provider) => provider.status === "error" && provider.checkedAt !== initialCheckedAt,
            );
        ...
        () =>
          Effect.gen(function* () {
            ...
            yield* Effect.gen(function* () {
              ...
              assert.strictEqual(cursorProvider?.message, "Cursor is disabled in Salchi settings.");
      ...
      it.effect("skips codex probes entirely when the provider is disabled", () =>
        Effect.gen(function* () {
          ...
          assert.strictEqual(status.installed, false);
          assert.strictEqual(status.message, "Codex is disabled in Salchi settings.");
        }),
      );
    });
    ...
    describe("checkClaudeProviderStatus", () => {
      ...
      it.effect("hides Claude Fable 5 before the supported Claude Code version", () =>
        Effect.gen(function* () {
          const status = yield* checkClaudeProviderStatus(
            defaultClaudeSettings,
            claudeCapabilities(),
      ...
      it.effect("hides Claude Opus 4.8 before the supported Claude Code version", () =>
        Effect.gen(function* () {
          const status = yield* checkClaudeProviderStatus(
            defaultClaudeSettings,
            claudeCapabilities(),
      ...
      it.effect("hides version-gated Claude Opus models on older Claude Code versions", () =>
        Effect.gen(function* () {
          const status = yield* checkClaudeProviderStatus(
            defaultClaudeSettings,
            claudeCapabilities(),
      ...
      it.effect("runs Claude status probes with the configured Claude HOME", () => {
        const claudeHome = "/tmp/salchi-claude-home";

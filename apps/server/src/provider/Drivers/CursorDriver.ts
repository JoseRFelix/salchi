 ...
 * hook to republish maintenance metadata without blocking the initial
 * `ready`-state publish.
 *
 ...
 */
import { CursorSettings, ProviderDriverKind, type ServerProvider } from "@salchi/contracts";
...
import { BrowserAgentBroker } from "../../browser/Services/BrowserAgentBroker.ts";
...
export const CursorDriver: ProviderDriver<CursorSettings, CursorDriverEnv> = {
  ...
  create: ({ instanceId, displayName, accentColor, environment, enabled, config }) =>
    Effect.gen(function* () {
      ...
      const browserAgentBroker = yield* BrowserAgentBroker;
      ...
      const snapshot = yield* makeManagedServerProvider<CursorSettings>({
        ...
        checkProvider,
        // Cursor model discovery runs during provider checks. Enrichment only
        // republishes maintenance metadata such as version advisories.

  ...
  Effect.gen(function* () {
    ...
    const runtime = ManagedRuntime.make(
      OtlpTracer.layer({
        ...
        resource: {
          serviceName: "salchi-web",
          attributes: {
            "service.runtime": "salchi-web",
...
const buildAppUnderTest = (options?: {
  ...
  Effect.gen(function* () {
    ...
    const servedRoutesLayer = HttpRouter.serve(makeRoutesLayer, {
      ...
      Layer.provide(Layer.merge(providerRegistryLayer, providerMaintenanceRunnerLayer)),
      Layer.provide(
        Layer.mock(ProviderService)({
          startSession: () => Effect.die("Unsupported provider service call in test"),
          sendTurn: () => Effect.die("Unsupported provider service call in test"),
          interruptTurn: () => Effect.die("Unsupported provider service call in test"),
          respondToRequest: () => Effect.die("Unsupported provider service call in test"),
      ...
      Layer.provide(
        Layer.mock(ProjectionSnapshotQuery)({
          ...
          getMostRecentActiveThreadIdByProjectId: () => Effect.succeed(Option.none()),
...
it.layer(NodeServices.layer)("server router seam", (it) => {
  ...
  it.effect("serves project favicon requests before the dev URL redirect", () =>
    Effect.gen(function* () {
      ...
      const projectDir = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "salchi-router-project-favicon-",
      });
  ...
  it.effect("serves the fallback project favicon when no icon exists", () =>
    Effect.gen(function* () {
      const fileSystem = yield* FileSystem.FileSystem;
      const projectDir = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "salchi-router-project-favicon-fallback-",
      });
  ...
  it.effect("serves the public environment descriptor without requiring auth", () =>
    Effect.gen(function* () {
      ...
      const response = yield* HttpClient.get("/.well-known/salchi/environment");
  ...
  it.effect("reports unauthenticated session state without requiring auth", () =>
    Effect.gen(function* () {
      ...
      assert.isTrue(body.auth.sessionCookieName.startsWith("salchi_session_"));
    }).pipe(Effect.provide(NodeHttpServer.layerTest)),
  );
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        assert.equal(bootstrapResponse.status, 200);
        assert.equal(bootstrapBody.authenticated, true);
        assert.equal(bootstrapBody.sessionMethod, "bearer-access-token");
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        const fileSystem = yield* FileSystem.FileSystem;
        const projectDir = yield* fileSystem.makeTempDirectoryScoped({
          prefix: "salchi-router-project-favicon-query-token-",
        });
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const wsUrl = `${yield* getWsServerUrl("/ws", { authenticated: false })}?ticket=${encodeURIComponent(wsTicketBody.ticket)}`;
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        assert.equal(record.resourceAttributes["service.name"], "salchi-web");
  ...
  it.effect("rejects websocket rpc handshake when session authentication is missing", () =>
    Effect.gen(function* () {
      ...
      const workspaceDir = yield* fs.makeTempDirectoryScoped({
        prefix: "salchi-ws-auth-required-",
      });
  ...
  it.effect("routes websocket rpc projects.searchEntries", () =>
    Effect.gen(function* () {
      ...
      const path = yield* Path.Path;
      const workspaceDir = yield* fs.makeTempDirectoryScoped({
        prefix: "salchi-ws-project-search-",
      });
      yield* fs.writeFileString(
        path.join(workspaceDir, "needle-file.ts"),
        "export const needle = 1;",
  ...
  it.effect("routes websocket rpc projects.searchEntries marks gitignored files", () =>
    Effect.gen(function* () {
      ...
      const path = yield* Path.Path;
      const workspaceDir = yield* fs.makeTempDirectoryScoped({
        prefix: "salchi-ws-project-search-gitignored-",
      });
  ...
  it.effect("routes websocket rpc projects.listDirectoryEntries", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const workspaceDir = yield* fs.makeTempDirectoryScoped({
        prefix: "salchi-ws-project-list-directory-",
      });
      yield* fs.makeDirectory(path.join(workspaceDir, "src"), { recursive: true });
      ...
      const response = yield* Effect.scoped(
        withWsRpcClient(wsUrl, (client) =>
          client[WS_METHODS.projectsListDirectoryEntries]({
            cwd: workspaceDir,
            limit: 10,
          }),
        ),
      ...
      assert.deepEqual(
        response.entries.map((entry) => `${entry.kind}:${entry.path}`),
  ...
  it.effect("routes websocket rpc projects.writeFile", () =>
    Effect.gen(function* () {
      ...
      const path = yield* Path.Path;
      const workspaceDir = yield* fs.makeTempDirectoryScoped({
        prefix: "salchi-ws-project-write-",
      });
  ...
  it.effect("routes websocket rpc projects.deleteEntry for files", () =>
    Effect.gen(function* () {
      ...
      const path = yield* Path.Path;
      const workspaceDir = yield* fs.makeTempDirectoryScoped({
        prefix: "salchi-ws-project-delete-",
      });
  ...
  it.effect("routes websocket rpc projects.readFile", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const workspaceDir = yield* fs.makeTempDirectoryScoped({ prefix: "salchi-ws-project-read-" });
      yield* fs.makeDirectory(path.join(workspaceDir, "src"), { recursive: true });
      yield* fs.writeFileString(path.join(workspaceDir, "src", "index.ts"), "export {};\n");
  ...
  it.effect("creates a missing workspace root during websocket project.create dispatch", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const parentDir = yield* fs.makeTempDirectoryScoped({ prefix: "salchi-ws-project-create-" });
  ...
  it.effect("enriches replayed project events with repository identity metadata", () =>
    Effect.gen(function* () {
      const repositoryIdentity = {
        canonicalKey: "github.com/salchi/salchi",
        locator: {
          source: "git-remote" as const,
          remoteName: "origin",
          remoteUrl: "git@github.com:Salchi/salchi.git",
        },
        displayName: "Salchi/salchi",
        provider: "github",
        owner: "Salchi",
        name: "salchi",
      };
      ...
      Effect.gen(function* () {
        ...
        const refreshStatus = vi.fn((_: string) =>
          Effect.succeed({
            ...
            hasPrimaryRemote: true,
            isDefaultRef: false,
            refName: "salchi/bootstrap-refName",
            hasWorkingTreeChanges: false,
            workingTree: emptyWorkingTree(),
            hasUpstream: true,
            aheadCount: 0,
            ...
            client[ORCHESTRATION_WS_METHODS.dispatchCommand]({
              ...
              bootstrap: {
                ...
                prepareWorktree: {
                  ...
                  baseBranch: "main",
                  branch: "salchi/bootstrap-refName",
                },
                runSetupScript: true,
              },
              createdAt,
        ...
        assert.deepEqual(createWorktree.mock.calls[0]?.[0], {
          cwd: "/tmp/project",
          refName: "main",
          newRefName: "salchi/bootstrap-refName",
          path: null,
        });

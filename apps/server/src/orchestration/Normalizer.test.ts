...
it.layer(TestLayer)("normalizeDispatchCommand image attachments", (it) => {
  ...
  it.effect("persists uploaded PDF data URLs and strips upload-only data", () =>
    Effect.gen(function* () {
      const fileSystem = yield* FileSystem.FileSystem;
      const serverConfig = yield* ServerConfig;
      const normalized = yield* normalizeDispatchCommand(
        pdfUploadCommand("data:application/pdf;base64,JVBERg=="),
      );
      ...
      assert.isString(attachmentPath);
      assert.match(attachmentPath!, /\.pdf$/);
      const fileInfo = yield* fileSystem.stat(attachmentPath!);
      assert.equal(fileInfo.type, "File");
      assert.equal(Number(fileInfo.size), 4);
    }),
  ...
  it.effect("persists uploaded PDF data URLs for queued turns", () =>
    Effect.gen(function* () {
      const fileSystem = yield* FileSystem.FileSystem;
      const serverConfig = yield* ServerConfig;
      const normalized = yield* normalizeDispatchCommand(
        pdfUploadCommand("data:application/pdf;base64,JVBERg==", "thread.turn.queue"),
      );
  ...
  it.effect("rejects invalid, malformed, empty, and oversized PDF uploads", () =>
    Effect.gen(function* () {
      const oversizedBytes = new Uint8Array(PROVIDER_SEND_TURN_MAX_PDF_BYTES + 1);
      oversizedBytes.fill(1);

...
export interface DesktopEnvironmentShape {
  ...
  readonly devRemoteSalchiServerEntryPath: Option.Option<string>;
...
>()("@salchi/desktop/app/DesktopEnvironment") {}
...
const makeDesktopEnvironment = Effect.fn("desktop.environment.make")(function* (
  ...
  const baseDir = Option.getOrElse(config.salchiHome, () => path.join(homeDirectory, ".salchi"));
  ...
  const userDataDirName = isDevelopment ? "salchi-dev" : "salchi";
  const legacyUserDataDirName = isDevelopment ? "Salchi (Dev)" : "Salchi (Alpha)";
  ...
  return DesktopEnvironment.of({
    ...
    devServerUrl,
    devRemoteSalchiServerEntryPath: config.devRemoteSalchiServerEntryPath,
    ...
    branding,
    displayName,
    appUserModelId: isDevelopment ? "com.joserfelix.salchi.dev" : "com.joserfelix.salchi",
    linuxDesktopEntryName: isDevelopment ? "salchi-dev.desktop" : "salchi.desktop",
    linuxWmClass: isDevelopment ? "salchi-dev" : "salchi",
    userDataDirName,
    ...
    developmentDockIconPath: path.join(rootDir, "assets", "salchi", "salchi-icon-1024.png"),
  });
});

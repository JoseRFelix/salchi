...
const AppPackageMetadata = Schema.Struct({
  salchiCommitHash: Schema.optional(Schema.String),
});
...
>()("@salchi/desktop/app/DesktopAppIdentity") {}
    ...
    return yield* Option.match(raw, {
      ...
      onSome: (value) =>
        decodeAppPackageMetadata(value).pipe(
          Effect.map((parsed) =>
            Option.fromNullishOr(parsed.salchiCommitHash).pipe(Option.flatMap(normalizeCommitHash)),
          ),

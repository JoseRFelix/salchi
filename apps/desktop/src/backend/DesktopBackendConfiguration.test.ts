    ...
    Layer.provide(
      Layer.mergeAll(
        ...
        DesktopConfig.layerTest({
          ...
          SALCHI_PORT: "9999",
          SALCHI_MODE: "desktop",
          SALCHI_DESKTOP_LAN_HOST: "192.168.1.50",
...
const withHarness = <A, E, R>(
  ...
  Effect.gen(function* () {
    ...
    const baseDir = yield* fileSystem.makeTempDirectoryScoped({
      prefix: "salchi-desktop-backend-config-test-",
    });
  ...
  it.effect("resolves backend start config with a stable scoped bootstrap token", () =>
    withHarness(
      Effect.gen(function* () {
        ...
        assert.isUndefined(first.env.SALCHI_PORT);
        assert.isUndefined(first.env.SALCHI_MODE);
        assert.isUndefined(first.env.SALCHI_DESKTOP_LAN_HOST);
        ...
        assert.equal(first.bootstrap.salchiHome, environment.baseDir);

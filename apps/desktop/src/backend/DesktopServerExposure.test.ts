...
function makeEnvironmentLayer(baseDir: string, env: Record<string, string | undefined> = {}) {
  return makeDesktopEnvironmentLayer({
    ...
    Layer.provide(
      Layer.mergeAll(NodeServices.layer, DesktopConfig.layerTest({ SALCHI_HOME: baseDir, ...env })),
    ),
  );
}
...
function makeLayer(input: {
  ...
  const env = { SALCHI_HOME: input.baseDir, ...input.env };
...
const withHarness = <A, E, R>(
  ...
  Effect.gen(function* () {
    ...
    const baseDir = yield* fileSystem.makeTempDirectoryScoped({
      prefix: "salchi-desktop-server-exposure-test-",
    });
...
describe("DesktopServerExposure", () => {
  ...
  it.effect("uses ConfigProvider desktop exposure overrides", () =>
    withHarness(
      ...
      {
        SALCHI_DESKTOP_LAN_HOST: "10.0.0.7",
        SALCHI_DESKTOP_HTTPS_ENDPOINTS: "https://public.example.test",
      },
    ),
  );
  ...
  it.effect("advertises loopback, LAN, and configured manual endpoints from runtime state", () =>
    withHarness(
      ...
      {
        SALCHI_DESKTOP_HTTPS_ENDPOINTS:

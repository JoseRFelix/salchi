...
const ROUTER_HOST = "app.salchi.codes";
const HOSTED_WEB_CHANNEL_COOKIE = "salchi_web_channel";
const LATEST_ORIGIN = "https://latest.app.salchi.codes";
const NIGHTLY_ORIGIN = "https://nightly.app.salchi.codes";
...
export const config: VercelConfig = {
  buildCommand:
    'vp run --filter @salchi/web build && node ../../scripts/apply-web-brand-assets.ts --channel "${VITE_HOSTED_APP_CHANNEL:-latest}"',
  git: {
    deploymentEnabled: false,
  },
  installCommand:
    "npm install -g vite-plus && vp install --filter '@salchi/scripts...' --filter '@salchi/web...'",
  routes: [
    {
      src: "/__salchi/channel",

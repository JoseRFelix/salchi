...
describe("Pierre file icons", () => {
  ...
  it("extends Pierre with Salchi-specific exact filename icons", () => {
    assert.equal(
    ...
    assert.equal(
      ...
      "salchi-file-icon-tsconfig",
    );
    assert.equal(resolvePierreIconForEntry("AGENTS.md", "file")?.name, "salchi-file-icon-agents");
    assert.equal(resolvePierreIconForEntry("CLAUDE.md", "file")?.name, "salchi-file-icon-claude");
    assert.equal(resolvePierreIconForEntry("README.md", "file")?.name, "salchi-file-icon-readme");
    assert.equal(
      resolvePierreIconForEntry("pnpm-lock.yaml", "file")?.name,
      "salchi-file-icon-pnpm",
    );
    assert.equal(
  ...
  it("ships every custom icon referenced by the extended resolver", () => {
    const customIconNames = new Set(Object.values(SALCHI_PIERRE_ICONS.byFileName));
    for (const iconName of customIconNames) {
      assert.include(SALCHI_PIERRE_ICONS.spriteSheet, `id="${iconName}"`);
    }
  });

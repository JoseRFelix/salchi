...
function ChatThreadRouteView() {
  ...
  const openDiff = useCallback(() => {
    ...
    void navigate({
      to: "/$environmentId/$threadId",
      params: buildThreadRouteParams(threadRef),
      search: (previous) => buildOpenDiffSearch(previous, { source }),
    });
  ...
  useMobileEdgeSwipe({
    ...
    enabled: shouldUseDiffSheet && activeRightPanelView === null && !leftSidebarOpenMobile,
  ...
  useMobileEdgeSwipe({
    action: "close",
    enabled: shouldUseDiffSheet && activeRightPanelView !== null,
  ...
  // Mobile releases the expensive parsed/rendered diff tree on dismiss. The
  // panel shell retains only its bounded, cacheless worker briefly so a rapid
  // reopen does not churn WebKit worker heaps.

import { EnvironmentId, ProjectId, ProviderInstanceId, ThreadId } from "@salchi/contracts";
...
describe("formatWorktreePathForDisplay", () => {
  it("shows only the last path segment for unix-like paths", () => {
    const result = formatWorktreePathForDisplay(
      "/Users/julius/.salchi/worktrees/salchi-mvp/salchi-4e609bb8",
    );
    expect(result).toBe("salchi-4e609bb8");
  });

  it("normalizes windows separators before selecting the final segment", () => {
    const result = formatWorktreePathForDisplay(
      "C:\\Users\\julius\\.salchi\\worktrees\\salchi-mvp\\salchi-4e609bb8",
    );
    expect(result).toBe("salchi-4e609bb8");
  });

  it("uses the final segment even when outside ~/.salchi/worktrees", () => {

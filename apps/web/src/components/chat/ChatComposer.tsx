...
export const ChatComposer = memo(function ChatComposer(props: ChatComposerProps) {
  ...
  const composerPreviewAnnotationIdSet = useMemo(
    () => new Set(composerPreviewAnnotations.map((annotation) => annotation.id)),
        ...
        >
          ...
          >
            ...
            {!isComposerCollapsedMobile &&
              !isComposerApprovalState &&
              pendingUserInputs.length === 0 &&
              composerImages.some((image) => !composerPreviewAnnotationIdSet.has(image.id)) && (
                <div className="mb-3 flex flex-wrap gap-2">
                  {composerImages
                    .filter((image) => !composerPreviewAnnotationIdSet.has(image.id))
                    .map((image) => (
                      <div
                        key={image.id}

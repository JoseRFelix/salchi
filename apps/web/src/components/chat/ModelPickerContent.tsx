...
import { resolveSelectableModel } from "@salchi/shared/model";
...
export const ModelPickerContent = memo(function ModelPickerContent(props: {
  ...
  // On phones the picker is a full-screen modal. Auto-focusing the search
  // input there force-opens the soft keyboard on mount, which both hides
  // most of the list and makes dismissing the keyboard feel like it closes
  // the modal. Let the user tap the search field to filter instead.
  ...
  useEffect(() => {
    ...
    if (selectedInstanceId === "favorites") return;
    const selectedEntry = entryByInstanceId.get(selectedInstanceId);
    if (!selectedEntry || !matchesLockedProvider(selectedEntry)) {
  ...
  return (
    <TooltipProvider delay={0}>
      <div
        className={cn(
          "relative flex h-screen max-h-96 w-screen max-w-100 flex-col overflow-hidden rounded-lg border bg-popover not-dark:bg-clip-padding text-popover-foreground shadow-lg/5 before:pointer-events-none before:absolute before:inset-0 before:rounded-[calc(var(--radius-lg)-1px)] before:shadow-[0_1px_--theme(--color-black/4%)] dark:before:shadow-[0_-1px_--theme(--color-white/6%)]",
          "max-sm:h-dvh max-sm:max-h-none max-sm:w-screen max-sm:max-w-none max-sm:rounded-none max-sm:border-0 max-sm:shadow-none max-sm:before:hidden max-sm:pt-safe max-sm:pb-safe max-sm:pl-safe max-sm:pr-safe",
        )}
        data-model-picker-content="true"
      >
        {/* Mobile-only header: keeps the full-screen modal legible below the
            status bar and gives an explicit way out without selecting. */}
        <div className="hidden max-sm:flex shrink-0 items-center justify-between gap-2 border-b px-3 py-2.5">
              ...
              <div className="px-4 pt-2.5">
                <div className="-translate-y-px border-b border-border/70 pb-2.5 transition-colors focus-within:border-ring">
                  <ComboboxInput
                    ...
                    inputClassName="rounded-none bg-transparent text-sm max-sm:text-[15px]"
              ...
              <div className="relative min-h-0 flex-1 overflow-hidden">
                <ComboboxListVirtualized className="model-picker-list size-full min-w-0 p-0">
                  <LegendList<string>
                    ...
                    className={cn(
                      "scrollbar-gutter-both h-full overflow-x-hidden overscroll-y-contain py-1 sm:py-1.5 [--fade-size:1.5rem]",

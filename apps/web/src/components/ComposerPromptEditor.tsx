...
import { type ServerProviderSkill } from "@salchi/contracts";
...
function ComposerCommandKeyPlugin(props: {
  ...
  useEffect(() => {
    const handleCommand = (
      ...
      if (key === "Enter" && isComposerNativeComposingKeyEvent(event)) {
      ...
      <div className="relative">
        <PlainTextPlugin
          contentEditable={
            <ContentEditable
              className={cn(
                "block max-h-50 min-h-14 w-full overflow-y-auto whitespace-pre-wrap wrap-break-word bg-transparent text-[16px] leading-relaxed text-foreground focus:outline-none sm:min-h-17.5 sm:text-[15px] md:text-[14px]",
                className,
              )}
          ...
          placeholder={
            terminalContexts.length > 0 ? null : (
              <div className="pointer-events-none absolute inset-0 text-[16px] leading-relaxed text-muted-foreground/35 sm:text-[15px] md:text-[14px]">
                {placeholder}
              </div>
            )
          }
...
export function ComposerPromptEditor({
  ...
  const initialConfig = useMemo<InitialConfigType>(
    () => ({
      namespace: "salchi-composer-editor",
      editable: true,
